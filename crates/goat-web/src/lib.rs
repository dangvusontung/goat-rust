...
fn beat_to_dto(
    ...
    BeatDto {
        ...
        // Match Flow engine: dynamic beat count — moments-so-far + estimated
        // remaining ~5-minute flow ticks (mirrors the bridge's DTO mapping).
        beat_number: ms.moments.len() + 1,
        total_beats: ms.moments.len() + ((90 - ms.minute.min(90)) / 5).max(1) as usize,
...
fn build_match_setup(
    ...
    // `MatchSetup.opp_name` is `&'static str`; the demo leaks one copy per
    // match (bounded, WASM session-scoped).
    let opp_name: &'static str = Box::leak(opp_name.to_string().into_boxed_str());
    let pc_club_id = s.pc_club_idx as u32;
    MatchSetup {
        ...
        staff_mods: goat_world::staff::club_staff_mods(own_strength),
