...
fn golden_lifestyle_seed3() {
    ...
    // peak OVR drops 88 → 64. Injury counts unchanged (durability roll untouched).
    // Pro-dominates-Flashy ordering (higher peak, far fewer injuries) still holds.
    let golden = [
        (PROFESSIONAL, 64, 66),
        (1u8, 64, 113), // Balanced — must match pre-lifestyle behavior
        (FLASHY, 62, 158),
    ];
