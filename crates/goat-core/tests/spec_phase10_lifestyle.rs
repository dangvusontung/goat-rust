...
fn golden_lifestyle_seed3() {
    ...
    // RE-FROZEN for the ceiling-lottery restore (intentional): seed 3 now rolls a
    // ceiling in 70–99 instead of always 99 — this seed draws a modest ceiling, so
    // peak OVR drops 88 → 64.
    // RE-FROZEN again at the PA2/origin-main merge (intentional): the merged tree
    // combines the lottery restore with the local training-intensity ceiling work,
    // shifting injury counts (Pro/Balanced 66/113 → 101/101, Flashy 158 → 157).
    // Pro-dominates-Flashy ordering (higher peak, fewer injuries) still holds.
    let golden = [
        (PROFESSIONAL, 64, 101),
        (1u8, 64, 101), // Balanced — must match pre-lifestyle behavior
        (FLASHY, 62, 157),
    ];
