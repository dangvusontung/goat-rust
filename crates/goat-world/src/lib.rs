...
//!
//! Two world representations coexist post-merge:
//! - `world` (origin/main): `WorldGenesis` — 20 real nations × 3 tiers × 20 clubs,
//!   generated per `world_seed`, with per-club tactical identity, budgets and
//!   academy boosts. Drives the Round5 economy/scouting/transfer/competition
//!   modules, the save format, and the bridge/web front-ends.
//! - `layout` + `worldgen` + `nations` (local PA2/world-scale-up line): the
//!   50-nation / 2,544-club structural world the local career tooling
//!   (career-sim, career-batch, orbit overlay, staff) was built on.
...
// Local-line structural world (50 nations / 2,544 clubs) — only the names that
// do NOT collide with `world`'s genesis exports are re-exported at the root;
// the colliding constants (`NUM_CLUBS`, `CLUBS_PER_DIV`, …) stay path-qualified
// under `layout::`.
pub use layout::{
    club_div_pos, club_division, div_clubs, div_index, facilities_mult, nation_name, NATION_BRAZIL,
    NATION_ENGLAND,
};
pub use manager::{
