...
/// Orbit-aware sibling of `batch_tick_season` (PA2 M3 — same zero-ripple
/// pattern as `batch_tick_season_with_match_points`): divisions listed in
/// `orbit` credit each player only the REMAINDER of his season apps/goal share
/// after his real deep-simmed residue, and the top-scorer race counts orbit
/// goals — the division's scoring chart reflects what actually happened in the
/// PC's matches. Youth intake is NOT run here (unlike the local line's
/// original): callers route replenishment through `population::apply_youth_intake`.
