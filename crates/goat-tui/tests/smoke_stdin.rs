...
fn seed_save_at_season_end(dir: &std::path::Path) {
    ...
    // Avg output 30 (not 50): the merged offer gate scouts form+avg with ±8
    // noise (goat_world::scout) — 50 could scout up to 58 and fire the
    // interactive transfer window, consuming a scripted keypress; 30 can't
    // clear the 55 bar even on a maximum upward read.
    state.pc_season_output = 300;
...
fn continue_break_week_elapses_without_a_keypress() {
    ...
    // Post-merge the merged sim (durability trait + intensity ceilings) puts a
    // scripted injury stop in this run — one extra [C]/defer pair rides through
    // the event stop before the round-6 offer.
    let script = format!(
        "{}{}K\nK\nK\nK\nK\nC\nX\nC\nX\nQ\n",
        new_game_reaching_menu(),
        through_pre_season()
